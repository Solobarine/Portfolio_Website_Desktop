...
function storage() { const dataStorage = { 
  name: theName.value, 
...
window.addEventListener('load', () => { const restored = JSON.parse(localStorage.getItem('data'));
  theName.value = restored.name;
  theEmail.value = restored.email;
  theMsg.value = restored.msg;
});